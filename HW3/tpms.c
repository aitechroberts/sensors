...
#define PARSE_BUFFER_SIZE 512 // 1 message could be up to 256 bytes so double that to start and adjust as necessary for message buffering
...
static void send_response(const uint8_t *payload, int payload_len) {
    // total length in bytes = 1 (Len) + 1 (CRC) + 2 (DevID) + payload_len + 2 (EOL,EOL)
	...
	// Neat idea on index. initialize at 0 and increment value inside array assignment as you go
	// Found online, and basically means assign index in array THEN increment value but one-liner
    int idx = 0;
...
static void handle_message_payload(const uint8_t *payload, int payload_len) {
    ...
    if(command == 0x01) {
        if(payload_len < 2) {
            // Not enough data for a PING token
            return;
        }
        uint8_t token = payload[1];
        uint8_t response[2];
        response[0] = 0x01;         // same command
        response[1] = token + 1;    // rollover automatically in 8-bit
